...
def get_adj_matrix(x, edge_index) -> np.ndarray:
    ...
    :param x: the graph nodes
    :param edge_index:all edges of the graph
    :return:adjacency matrix
    """
    num_nodes = x.shape[0]
    adj_matrix = np.zeros(shape=(num_nodes, num_nodes))  # matrix: nXn
    for i, j in zip(edge_index[0], edge_index[1]):  # all connections
        adj_matrix[i, j] = 1.
    return adj_matrix
...
def get_heat_matrix(
    ...
    A_tilde = adj_matrix + np.eye(num_nodes)  # np.eye is identity matrix, adding a self-edge?
...
def get_top_k_matrix(A: np.ndarray, k: int = 128) -> np.ndarray:
    ...
    """
    num_nodes = A.shape[0]
    row_idx = np.arange(num_nodes)  # [0,1,...]
...
def get_PPR_adj(x, edge_index, alpha=0.05, k=None, eps=None):
    ...
    :return:
    """
    assert ((k is None and eps is not None) or (k is not None and eps is None))
